...
def all_requests():
    ...
    click.echo(click.style(f"\n\n\n{rec_requests}\n\n{sent_requests}\n\n\n", bg='red', fg='white'))
    return {'sent_requests': [request.to_dict() for request in sent_requests], "rec_requests": [request.to_dict() for request in rec_requests]}


@request_routes.route('/send/<int:user_id>', methods=['POST'])
@login_required
def send_request(user_id):
    request = Request(receiver_id=user_id, sender_id=current_user.get_id(), is_approved=False)
    click.echo(click.style(f"\n\n\n{request}\n\n", bg='blue', fg='white'))
...
@request_routes.route('/<int:req_id>/decline/<int:user_id>', methods=['DELETE'])
@login_required
def decline_request(req_id, user_id):
    request = Request.query.filter(Request.id == req_id).first()
    click.echo(click.style(f"\n\n\n{request}\n\n", bg='red', fg='white'))
    db.session.delete(request)
    db.session.commit()
